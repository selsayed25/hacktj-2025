...
for folder in [UPLOAD_FOLDER, PROCESSED_FOLDER, AUDIO_FOLDER]:
...
def comprehensive_image_processing(image_path, unique_id):
    ...
    # 1) Basic processing - Otsu's thresholding (https://docs.opencv.org/4.x/d7/d4d/tutorial_py_thresholding.html)
    ...
    # 3) Denoising (https://docs.opencv.org/4.x/d5/d69/tutorial_py_non_local_means.html)
    ...
    # 5) Contrast enhancement (https://www.geeksforgeeks.org/image-enhancement-techniques-using-opencv-python/)
    ...
    # 7) Dilation and Erosion (https://docs.opencv.org/4.11.0/db/df6/tutorial_erosion_dilatation.html)
    ...
    # 8) Sharpen image (https://stackoverflow.com/questions/4993082/how-can-i-sharpen-an-image-in-opencv)
    ...
    # 9) Try inverting the image (sometimes helps with white text on dark backgrounds)
    ...
    # 10) Scale the image (can help with very small text)
    ...
    try:
        ...
        # Get the largest contour areas
        contours = sorted(contours, key=cv2.contourArea, reverse=True)[:5]
        
        # Try to find the angle of the largest contour
...
def extract_best_text(processed_versions, language='eng'):
    """Extract text from all processed versions and return the best result"""
    ...
    # Custom config with extra parameters for better accuracy
    for version_name, img_path in processed_versions:
        for psm in psm_modes:
            try:
                # Custom config with various OCR engine modes and PSM values
                ...
                # Calculate average confidence if words were found
                avg_confidence = confidence_sum / word_count if word_count > 0 else 0
                
                # Keep track of the method with highest confidence
